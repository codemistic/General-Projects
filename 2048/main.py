...
class GameDisplay(Frame):
    def __init__(self):
        ...
        self.commands = {
            ctrls1[0]: func.move_up,
            ctrls1[1]: func.move_left,
            ctrls1[2]: func.move_down,
            ctrls1[3]: func.move_right,
            ctrls2[0]: func.move_up,
            ctrls2[1]: func.move_left,
            ctrls2[2]: func.move_down,
            ctrls2[3]: func.move_right,
        }
            ...
            for j in range(grid_coord[1]):
                if self.M[i][j]:
                    self.cells[i][j].configure(
                        text=str(self.M[i][j]),
                        bg=bg_cell[self.M[i][j]],
                        fg=cell_clr[self.M[i][j]],
                    )
                else:
